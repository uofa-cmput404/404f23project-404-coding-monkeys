...
class PostCreate(CreateView):
    ...
    def form_valid(self, form):
        ...
        unique_id = form.cleaned_data.get('uuid')
        ...
        if picture is not None:
            ...
            image_base64, contentType_pic = get_picture_info(picture)
...
def make_new_post(request, form=None):
    if request.method == 'GET':
        try:
            authors = AuthorUser.objects.get_queryset()
...
def update_or_create_post(request, post_uuid):
    ...
    post.sharedWith = request.POST.get('sharedWith')
    ...
    if request.FILES.get('picture') is not None or not newPost:
...
def edit_post(request, author_id, post_uuid):
    if request.method == 'GET':
        ...
        try:
            ...
            # TODO raise error popup
            return redirect('stream')

        # deal with updating picture
        try:
            unique_id_pic = str(post_uuid) + "_pic"
            post_pic = Posts.objects.get(uuid=unique_id_pic)

            # update existing pic post
            # TODO might need a case to delete the pic post if the user deletes the picture
            if request.FILES.get('picture') is not None:
            ...
            # otherwise make a new pic post if a picture was added
            if request.FILES.get('picture') is not None:
                # copy all fields related to post but change UUID and picture relevant fields
                post.uuid = unique_id_pic
                post.unlisted = True
                post.contentType = contentType_pic
                post.content = image_base64
